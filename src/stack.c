...
void upo_stack_push(upo_stack_t stack, void *data)
{
    ...
    upo_stack_node_t *newPtr = malloc(sizeof(upo_stack_node_t));
    if (newPtr == NULL)
    {
...
void upo_stack_pop(upo_stack_t stack, int destroy_data)
{
    ...
    {
        upo_stack_node_t *delPtr = stack->top;
...
void upo_stack_clear(upo_stack_t stack, int destroy_data)
{
    ...
    {
        upo_stack_node_t *tmpPtr = stack->top;
        stack->top = stack->top->next;
...
// Added by: akaAntox
void upo_stack_print(upo_stack_t stack)
{
    upo_stack_node_t *tmpPtr = stack->top;
    ...
    while (tmpPtr != NULL)
    {
        printf("%u", *(unsigned char *)tmpPtr->data);
        if (tmpPtr->next != NULL)
            printf(", ");
        tmpPtr = tmpPtr->next;
    }
